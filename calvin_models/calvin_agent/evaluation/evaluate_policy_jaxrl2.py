...
from calvin_agent.evaluation.single_chain_sequences import get_sequences_single_chain
...
EP_LEN = 360
NUM_SEQUENCES = int(os.environ.get("NUM_SEQUENCES", 5))
...
class CustomModel(CalvinBaseModel):
    def __init__(self, checkpoint_path, wandb_run_name, agent):
        # Initialize LCBC
        self.policy = jaxrl2_policy.Policy(checkpoint_path, wandb_run_name)
    ...
    def step(self, obs, goal):
        ...
        action_cmd = self.policy.predict_action(self.language_task, rgb_obs)
...
def evaluate_policy(model, env, epoch=0, eval_log_dir=None, debug=False, create_plan_tsne=False, single_chain=0):
    """
    ...
    if single_chain:
        eval_sequences = get_sequences_single_chain(NUM_SEQUENCES)
    else:
...
def main():
    ...
    parser.add_argument("--single_chain", default=0, type=int, help="Name of agent, (ddpmlc, idql, etc.).", required=True)
    ...
    if args.custom_model:
        ...
        evaluate_policy(model, env, debug=args.debug, single_chain=args.single_chain)
    else:
